...
# for (j in 1:50) {
  ...
  if (nsamples > 0) {
   ...
   if (entity$TYPE[j] == "concentration") {
     ...
     influx_source <- "copied from concentration"
  ...
  # if (entity$TYPE[j] == "pollen concentration") {
  else {
    ...
    # create and write and entity .csv file
   ...
   csvfile <- paste(entity_csv_path, entity_id, "_data.csv", sep="")
...
entity_list_filename <- "entity_list2.csv"
