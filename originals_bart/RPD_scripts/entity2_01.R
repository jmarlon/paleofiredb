# 3) create an "entity2" table, adding info from samples2
...
for (j in (1:nentities)) {
  ...
  check <- sample2$TYPE[sample2$ID_ENTITY == entity$ID_ENTITY[j]] 
  ...
  check <- sample2$analytical_sample_size[sample2$ID_ENTITY == entity$ID_ENTITY[j]] 
  entity2$analytical_sample_size[j] <- check[1]
  check <- sample2$analytical_sample_size_unit[sample2$ID_ENTITY == entity$ID_ENTITY[j]] 
  entity2$analytical_sample_size_unit[j] <- check[1]
  
  entity2$nsamp[j] <- length(check)
  check <- sample2$avg_depth[sample2$ID_ENTITY == entity$ID_ENTITY[j]] 
  entity2$min_depth[j] <- min(check, na.rm = TRUE)
  entity2$max_depth[j] <- max(check, na.rm = TRUE)
  check <- sample2$age[sample2$ID_ENTITY == entity$ID_ENTITY[j]] 
...
write.csv(entity2, file=entity_list_file, row.names = FALSE)
...
unique(sample2$ID_ENTITY[sample2$chronology == "none"]) # enitites without chronologies

# all entities
entities_in_sample2 <- unique(sample2$ID_ENTITY)
length(entities_in_sample2)

# outpath <- "/Users/bartlein/Projects/RPDv1b/RPD/RPDv1b/samples/"
# outfile <- "entities_in_sample2.csv"
# write.csv(entities_in_sample2, file=paste(outpath, outfile, sep=""), row.names = FALSE)
