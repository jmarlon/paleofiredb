# 5) fixups and records to exclude (e.g. zero char)  
...
# edit depths that are apparently in cm as opposed to meters
entity_id <- 1333
sample2$avg_depth[sample2$ID_ENTITY == entity_id]  <- sample2$avg_depth[sample2$ID_ENTITY == entity_id] * 100.0
writeLines("Edited depths that are apparently in cm as opposed to meters", con = fixup_file, sep = "\n")
writeLines("  ID_ENTITY = 1333", con = fixup_file, sep = "\n")

# edit sample with stray -999 value
sample_id <- 245460
sample2$quant[sample2$ID_SAMPLE == sample_id] <- 0.0
...
skip_id <- c(1459, 1461, 1605, 1606, 1623, 1627, 1628, 1730, 1750, 1775, 1923, 1934, 1937, 1938,
             1939, 1940, 1964, 1987, 1990, 1993, 1994, 1998, 2049, 2377, 2378, 2425, 2428, 2431, 2432)
entity2$skip[entity2$ID_ENTITY %in% skip_id] <- 1
writeLines("Set skip flag to 1 for entities without age info", con = fixup_file, sep = "\n")
writeLines("  ID_ENTITY = 1459, 1460, 1461, 1605, 1606, 1623, 1627, 1628, 1730, 1750, 1768, 1775, 1923, 1934, 1937, 1938,
             1939, 1940, 1964, 1987, 1990, 1993, 1994, 1998, 2049, 2377, 2378, 2425, 2428, 2431, 2432", con = fixup_file, sep = "\n")
...
entity2$skip[entity2$ID_ENTITY %in% skip_id] <- 1
writeLines("Set skip flag to 1 for entities with only one or two samples", con = fixup_file, sep = "\n")
writeLines("  ID_ENTITY = 85, 941, 1757, 1758, 1759, 1762, 1763, 1764, 1765, 1767, 1768, 1769, 1770, 1771, 1772, 1773", con = fixup_file, sep = "\n")

# set skip flag for ID_ENTITY values that appear in noncontiguous locations in database
skip_id <- c(1753)
entity2$skip[entity2$ID_ENTITY %in% skip_id] <- 1
writeLines("Set skip flag to 1 for or ID_ENTITY values that appear in noncontiguous locations in database", con = fixup_file, sep = "\n")
writeLines("  ID_ENTITY = 1753", con = fixup_file, sep = "\n")

# set skip flag for entities with zero charcoal throughout
skip_id <- c(1001, 1741, 1755, 1757, 1760, 1766)
entity2$skip[entity2$ID_ENTITY %in% skip_id] <- 1
writeLines("Set skip flag to 1 for entities with zero charcoal throughout", con = fixup_file, sep = "\n")
writeLines("  ID_ENTITY = 1001, 1741, 1755, 1757, 1760, 17662", con = fixup_file, sep = "\n")

# rewrite the entity2 table
